...
from pyqtgraph import  QMenuBar, PlotWidget, GraphicsObject, mkPen, mkBrush, mkQApp, AxisItem, LabelItem, QMainWindow
from pyqtgraph.Qt.QtWidgets import QGraphicsRectItem
from pyqtgraph.Qt.QtCore import QPointF, QRectF
from PyQt5.QtGui import QPicture, QPainter, QPixmap, QGraphicsPixmapItem, QGraphicsView
...
class DrawAreaScene(GraphicsScene):
    """ 描画ウインドウ用Scene """

    def __init__(self, *argv, **keywords):
        super(DrawAreaScene, self).__init__(*argv, **keywords)
        self.setBackgroundBrush((128, 128, 128, 255))  # 背景色を設定
        ...
        self.bgPixmap.fill((128, 128, 128, 255))
...
class DrawAreaView(QGraphicsView):
    ...
    def __init__(self, *argv, **keywords):
        ...
        self.setBackgroundBrush((64, 64, 64, 255))  # 背景色を設定
        ...
        self.oldCurPos = (0, 0)
        self.button =  QPushButton() #Qt.NoButton
        global zoomValue
        zoomValue = 100
    ...
    def mousePressEvent(self, event):
        ...
        if event.buttons() & pg.MidButton:
            # 中ボタンが押されてるならマウスカーソル形状を変更
            qApp.setOverrideCursor(pg.ClosedHandCursor)

    def mouseReleaseEvent(self, event):
        """ マウスボタンを離した時の処理 """
        pos = self.getMousePos(event, "Release")
        self.button = pg.NoButton
    ...
    def getMousePos(self, event, msg):
        ...
        kind = ""
        if event.buttons() & pg.LeftButton:
            kind = "Left "
        if event.button() & pg.MidButton:
            kind += "Mid "
        if event.button() & pg.RightButton:
            kind += "Right "
